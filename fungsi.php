...
function check_login($username, $password) {
    ...
    $connection = $db_object->get_connection(); // Asumsi metode ini mengembalikan objek koneksi
    ...
    if ($db_object->db_num_rows($query) > 0) {
        $user = $db_object->db_fetch_array($query);
...
// Fungsi untuk memulai sesi pengguna
...
// Fungsi untuk melindungi halaman yang membutuhkan login
...
// Fungsi untuk melindungi halaman yang membutuhkan peran tertentu
...
function open_connection()
{
    ...
    $dbname = "apriori"; // Ganti dengan nama database Anda
...
{
    label($label); // Memanggil fungsi label
    list_levels($name, $value, $required, $all, $text_all, $where, $params); // Memanggil fungsi list_levels
}
...
function list_levels($name, $selected_id = '', $required = false, $all = false, $text_all = '-', $where = '', $params = '')
{
    ...
    if ($required) echo "required='required'"; // Menambahkan atribut required jika diperlukan
    echo " class='form-control' " . $params . " >"; // Menambahkan kelas form-control

    if ($all) { // Menambahkan opsi semua/kosong jika diperlukan
        echo "<option value=''> $text_all </option>";
    }

    // Menambahkan opsi sesuai dengan level akses
    echo "<option value='1' ";
    if ($selected_id == "1") echo "selected='selected'"; // Menambahkan atribut selected jika ID sesuai
    echo ">Admin</option>";
    echo "<option value='2' ";
    if ($selected_id == "2") echo "selected='selected'"; // Menambahkan atribut selected jika ID sesuai
    echo ">User Store</option>";
...
// START TRANSAKSI
/**
 * Data list transaksi (from db: transaksi)
 * @param string $name
 * @param string $selected_id default=''
 * @param boolean $required default=false
 * @param boolean $all default=false
 ...
 * @param string $params default=''
 */
function list_transaksi($name, $selected_ids = [], $required = false, $all = false, $text_all = '-', $where = '', $params = '')
{
    $conn = open_connection();

    $query = "SELECT id, transaction_date, produk FROM transaksi";
    if ($where) {
        $query .= " WHERE $where";
    }
    ...
    while ($myrow = $result->fetch_assoc()) {
        ...
        if (in_array($myrow['id'], $selected_ids)) echo "selected='selected'";
        echo ">" . $myrow['transaction_date'] . " - " . $myrow['produk'] . "</option>";
    }
    echo "</select>";
...
function multi_checkbox($name, $values, $selected_ids = [])
{
    foreach ($values as $value => $label) {
        echo "<label><input type='checkbox' name='{$name}[]' value='$value' ";
        if (in_array($value, $selected_ids)) echo "checked";
        echo ">$label</label><br>";
    }
}
...
// START PROCESS LOG
/**
 * Data list process log (from db: process_log)
 * @param string $name
 * @param string $selected_id default=''
 * @param boolean $required default=false
 * @param boolean $all default=false
...
// START ITEMSET1
/**
 * Data list itemset1 (from db: itemset1)
 * @param string $name
 * @param string $selected_id default=''
 * @param boolean $required default=false
 * @param boolean $all default=false
 ...
 * @param string $params default=''
 */
function list_itemset1($name, $selected_id = '', $required = false, $all = false, $text_all = '-', $where = '', $params = '')
{
    $db_obj = new database(); // Membuat objek database (pastikan implementasi sudah sesuai dengan koneksi dan operasi database Anda)

    $table = "itemset1"; // Nama tabel itemset1
    $query = $db_obj->display_table_all_column($table, $where, false, false, 0, 0, 'id_process'); // Query untuk mengambil semua kolom dari tabel
    ...
    while ($myrow = $db_obj->db_fetch_array($query)) { // Melakukan loop pada hasil query
        echo "<option value='" . $myrow['id_process'] . "' ";
        if ($selected_id == $myrow['id_process']) echo "selected='selected'"; // Menambahkan atribut selected jika ID sesuai
        echo ">" . $myrow['atribut'] . " | Jumlah: " . $myrow['jumlah'] . " | Support: " . $myrow['support'] . " | Lolos: " . $myrow['lolos'] . "</option>"; // Menampilkan atribut, jumlah, support, dan lolos
    }
    echo "</select>";
}
// END ITEMSET1
...
// START CONFIDENCE
/**
 * Data list confidence (from db: confidence)
 * @param string $name
 * @param string $selected_id default=''
 * @param boolean $required default=false
 * @param boolean $all default=false
 ...
 * @param string $params default=''
 */
function list_confidence($name, $selected_id = '', $required = false, $all = false, $text_all = '-', $where = '', $params = '')
{
    $db_obj = new database(); // Membuat objek database (pastikan implementasi sudah sesuai dengan koneksi dan operasi database Anda)

    $table = "confidence"; // Nama tabel confidence
    $query = $db_obj->display_table_all_column($table, $where, false, false, 0, 0, 'id'); // Query untuk mengambil semua kolom dari tabel
    ...
    while ($myrow = $db_obj->db_fetch_array($query)) { // Melakukan loop pada hasil query
        echo "<option value='" . $myrow['id'] . "' ";
        if ($selected_id == $myrow['id']) echo "selected='selected'"; // Menambahkan atribut selected jika ID sesuai
        echo ">" . $myrow['confidence_value'] . "</option>"; // Menampilkan nilai confidence
    }
    echo "</select>";
}
// END CONFIDENCE
...
function format_date($date) {
    // Mengasumsikan format tanggal input adalah YYYY-MM-DD
...
function format_date2($date) {
    // Format tanggal untuk tampilan (misalnya: d-m-Y)
